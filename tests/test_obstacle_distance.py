...
class TestObstacleConstraint(unittest.TestCase):
    def testUnRotatedEllipsoid(self):
        ...
        constraint = obstacleConstraint(Sigma, np.zeros(3), obstacle, projection_matrix)
        self.assertLess(constraint, 0)
        constraint = obstacleConstraint(Sigma, -1*np.ones(3), obstacle, projection_matrix)
        self.assertGreater(constraint, 0)
        residual = obstacleResidual(Sigma, -1*np.ones(3), obstacle, projection_matrix)
        self.assertEqual(residual, 0)
        constraint = obstacleConstraint(Sigma, np.array([-1.2, 1, 0]), obstacle, projection_matrix)
        self.assertEqual(constraint, 0)
        constraint = obstacleConstraint(Sigma, np.array([1, 3.1, 0]), obstacle, projection_matrix)
        self.assertEqual(constraint, 0)
    ...
    def testRotatedEllipsoid(self):
        ...
        constraint = obstacleConstraint(Sigma, np.array([0, -1, -1]), obstacle, projection_matrix)
        self.assertGreater(constraint, 0)
        constraint = obstacleConstraint(Sigma, np.array([0, 1, 3.2]), obstacle, projection_matrix)
        self.assertEqual(constraint, 0)
        constraint = obstacleConstraint(Sigma, np.array([0, -1-0.2/np.sqrt(2), 1]), obstacle, projection_matrix)
